...
// #define ADC_DMA //WORKS
// #define WIFI_SOFT_AP //WORKS
// #define WIFI_STA //WORKS
#define TCP_CLIENT_ON // ERROR
// PREPR CFG//

#ifdef TCP_CLIENT_ON

#define CONFIG_EXAMPLE_IPV4 "192.168.0.106"
#define PORT 9000
...
#ifdef WIFI_STA
#define EXAMPLE_ESP_WIFI_SSID "DEV"
							   ...
							   int32_t event_id, void *event_data)
{
	if (event_id == WIFI_EVENT_AP_STACONNECTED)
	{
		...
		ESP_LOGI(WIFI_AP_TAG, "station" MACSTR "join, AID=%d", MAC2STR(event->mac),
				 event->aid);
	}
	else if (event_id == WIFI_EVENT_AP_STADISCONNECTED)
	{
		...
		ESP_LOGI(WIFI_AP_TAG, "station" MACSTR "leave, AID=%d", MAC2STR(event->mac),
				 event->aid);
	}
}
void wifi_init_softap(void)
{
	...
	ESP_LOGI(
		WIFI_AP_TAG, "wifi init_softap finished. SSID:%s password:%s channel:%d",
...
static adc_channel_t channel[1] = {
	ADC1_CHANNEL_0}; // channel attached to GPIO NUM 36
#endif
static const char *ADC_DMA_TAG = "ADC DMA";
static void continuous_adc_init(uint16_t adc1_chan_mask,
								uint16_t adc2_chan_mask, adc_channel_t *channel,
								uint8_t channel_num)
{
	adc_digi_init_config_t adc_dma_config = {
		.max_store_buf_size =
			128, // 256, //better response time due to faster buffer dumping
				 // //previously 1024bytes were allocated
	...
	for (int i = 0; i < channel_num; i++)
	{
		...
		ESP_LOGI(ADC_DMA_TAG, "adc_pattern[%d].atten is :%x", i,
				 adc_pattern[i].atten);
		ESP_LOGI(ADC_DMA_TAG, "adc_pattern[%d].channel is :%x", i,
				 adc_pattern[i].channel);
		ESP_LOGI(ADC_DMA_TAG, "adc_pattern[%d].unit is :%x", i,
				 adc_pattern[i].unit);
	}
...
if (wifi_ret == ESP_ERR_NVS_NO_FREE_PAGES ||
	wifi_ret == ESP_ERR_NVS_NEW_VERSION_FOUND)
{
...
if (wifi_ap_ret == ESP_ERR_NVS_NO_FREE_PAGES ||
	wifi_ap_ret == ESP_ERR_NVS_NEW_VERSION_FOUND)
{
	ESP_ERROR_CHECK(nvs_flash_erase());
...
#if CONFIG_IDF_TARGET_ESP32
			ESP_LOGI(ADC_DMA_TAG, "Unit: %d, Channel: %d, Value: %x", 1,
