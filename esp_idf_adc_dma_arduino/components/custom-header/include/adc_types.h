 ...
 *
 * @note  For ADC digital controller (DMA mode), ESP32 doesn't support
 * `ADC_UNIT_2`, `ADC_UNIT_BOTH`, `ADC_UNIT_ALTER`.
 */
typedef enum
{
 ...
 *
 * @note  For ESP32 ADC1, don't use `ADC_CHANNEL_8`, `ADC_CHANNEL_9`. See
 * ``adc1_channel_t``.
 */
typedef enum
{
...
/**
 * @brief ADC attenuation parameter. Different parameters determine the range of
 * the ADC. See ``adc1_config_channel_atten``.
 */
typedef enum
{
    ADC_ATTEN_DB_0 =
    ...
    ADC_ATTEN_DB_2_5 =
        1,               /*!<The input voltage of ADC will be attenuated extending the range of
                            measurement by about 2.5 dB (1.33 x) */
    ADC_ATTEN_DB_6 = 2,  /*!<The input voltage of ADC will be attenuated extending
                            the range of measurement by about 6 dB (2 x) */
    ADC_ATTEN_DB_11 = 3, /*!<The input voltage of ADC will be attenuated extending
                            the range of measurement by about 11 dB (3.55 x) */
    ADC_ATTEN_MAX,
} adc_atten_t;
...
typedef enum
{
    ...
    ADC_CONV_BOTH_UNIT =
        3,                   ///< Use Both ADC1 and ADC2 for conversion simultaneously
    ADC_CONV_ALTER_UNIT = 7, ///< Use both ADC1 and ADC2 for conversion by turn.
                             ///< e.g. ADC1 -> ADC2 -> ADC1 -> ADC2 .....
    ADC_CONV_UNIT_MAX,
...
typedef enum
{
    ADC_DIGI_FORMAT_12BIT __attribute__((
        deprecated)), /*!<ADC to DMA data format,                [15:12]-channel,
                         [11: 0]-12 bits ADC data (`adc_digi_output_data_t`).
                         Note: For single convert mode. */
    ADC_DIGI_FORMAT_11BIT __attribute__((
        deprecated)), /*!<ADC to DMA data format, [15]-adc unit, [14:11]-channel,
                         [10: 0]-11 bits ADC data (`adc_digi_output_data_t`).
 ...
 * @note  ESP32-S2: Member `channel` can be used to judge the validity of the
 * ADC data, because the role of the arbiter may get invalid ADC data.
 */
typedef struct
{
    union
    {
        ...
        struct
        {
            ...
            uint16_t channel : 4; /*!<ADC channel index info. For ESP32-S2:
                                      If (channel < ADC_CHANNEL_MAX), The data is
                                     valid. If (channel > ADC_CHANNEL_MAX), The data
                                     is invalid. */
...
typedef enum
{
    ADC_ARB_MODE_SHIELD, /*!<Force shield arbiter, Select the highest priority
                            controller to work. */
    ADC_ARB_MODE_FIX,    /*!<Fixed priority switch controller mode. */
    ADC_ARB_MODE_LOOP,   /*!<Loop priority switch controller mode. Each controller
                            has the same priority,   and the arbiter will switch to
                            the next controller after the measurement is completed.
                          */
} adc_arbiter_mode_t;
...
{
    adc_arbiter_mode_t
        mode;          /*!<Refer to ``adc_arbiter_mode_t``. Note: only support ADC2. */
...
#define ADC_ARBITER_CONFIG_DEFAULT()                                          \
    {                                                                         \
        .mode = ADC_ARB_MODE_FIX, .rtc_pri = 1, .dig_pri = 0, .pwdet_pri = 2, \
    }
...
typedef enum
{
    ADC_DIGI_FILTER_IDX0 =
        0,                /*!<The filter index 0.
                              For ESP32-S2, It can only be used to filter all enabled channels of
                             ADC1 unit at the same time. */
    ADC_DIGI_FILTER_IDX1, /*!<The filter index 1.
                              For ESP32-S2, It can only be used to filter all
                             enabled channels of ADC2 unit at the same time. */
...
typedef enum
{
    ...
    ADC_DIGI_FILTER_IIR_4,  /*!<The filter mode is first-order IIR filter. The
                               coefficient is 4. */
    ADC_DIGI_FILTER_IIR_8,  /*!<The filter mode is first-order IIR filter. The
                               coefficient is 8. */
    ADC_DIGI_FILTER_IIR_16, /*!<The filter mode is first-order IIR filter. The
                               coefficient is 16. */
    ADC_DIGI_FILTER_IIR_64, /*!<The filter mode is first-order IIR filter. The
                               coefficient is 64. */
...
{
    adc_unit_t
        adc_unit;                /*!<Set adc unit number for filter.
                                     For ESP32-S2, Filter IDX0/IDX1 can only be used to filter
                                    all enabled channels of ADC1/ADC2 unit at the same time. */
    adc_channel_t channel;       /*!<Set adc channel number for filter.
                                     For ESP32-S2, it's always `ADC_CHANNEL_MAX` */
    adc_digi_filter_mode_t mode; /*!<Set adc filter mode for filter. See
                                    ``adc_digi_filter_mode_t``. */
} adc_digi_filter_t;
...
typedef enum
{
    ADC_DIGI_MONITOR_IDX0 =
        0,                 /*!<The monitor index 0.
                              For ESP32-S2, It can only be used to monitor all enabled channels of
                              ADC1 unit at the same time. */
    ADC_DIGI_MONITOR_IDX1, /*!<The monitor index 1.
                              For ESP32-S2, It can only be used to monitor all
                              enabled channels of ADC2 unit at the same time. */
...
/*!<If ADC_OUT >  threshold, Generates monitor interrupt. */
    ...
    ADC_DIGI_MONITOR_LOW, /*!<If ADC_OUT <  threshold, Generates monitor
                             interrupt. */
#endif
...
{
    adc_unit_t
        adc_unit;          /*!<Set adc unit number for monitor.
                               For ESP32-S2, monitor IDX0/IDX1 can only be used to monitor
                              all enabled channels of ADC1/ADC2 unit at the same time. */
    adc_channel_t channel; /*!<Set adc channel number for monitor.
                               For ESP32-S2, it's always `ADC_CHANNEL_MAX` */
    adc_digi_monitor_mode_t
        mode; /*!<Set adc monitor mode. See ``adc_digi_monitor_mode_t``. */
#if CONFIG_IDF_TARGET_ESP32C3 || CONFIG_IDF_TARGET_ESP32H2
 ...
 *        Calculation formula: controller_clk = (`APLL` or `APB`) / (div_num +
 * div_a / div_b + 1).
 *
 * @note: The clocks of the DAC digital controller use the ADC digital
...
typedef struct
{
    ...
    uint32_t div_num; /*!<Division factor. Range: 0 ~ 255.
                          Note: When a higher frequency clock is used (the
                         division factor is less than 9), the ADC reading value
