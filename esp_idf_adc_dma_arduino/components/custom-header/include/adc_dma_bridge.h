...
/**** `adc1_channel_t` will be deprecated functions, combine into
 * `adc_channel_t` ********/
typedef enum {
  ...
  ADC1_CHANNEL_MAX,
    ...
    CONFIG_IDF_TARGET_ESP32S3  // TODO ESP32-S3 channels are wrong IDF-1776
/**** `adc1_channel_t` will be deprecated functions, combine into
 * `adc_channel_t` ********/
typedef enum {
...
typedef enum {
  ...
  ADC1_CHANNEL_MAX,
    ...
    CONFIG_IDF_TARGET_ESP32S3  // TODO ESP32-S3 channels are wrong IDF-1776
/**** `adc2_channel_t` will be deprecated functions, combine into
 * `adc_channel_t` ********/
typedef enum {
  ADC2_CHANNEL_0 =
...
// typedef enum {
//     ADC_ATTEN_DB_0   = 0,  /*!<No input attenumation, ADC can measure up to
//     approx. 800 mV. */ ADC_ATTEN_DB_2_5 = 1,  /*!<The input voltage of ADC
//     will be attenuated extending the range of measurement by about 2.5 dB
//     (1.33 x) */ ADC_ATTEN_DB_6   = 2,  /*!<The input voltage of ADC will be
//     attenuated extending the range of measurement by about 6 dB (2 x) */
//     ADC_ATTEN_DB_11  = 3,  /*!<The input voltage of ADC will be attenuated
//     extending the range of measurement by about 11 dB (3.55 x) */
//     ADC_ATTEN_MAX,
// } adc_atten_t;
/**
...
/**
 * The default (max) bit width of the ADC of current version. You can also get
 * the maximum bitwidth by `SOC_ADC_MAX_BITWIDTH` defined in soc_caps.h.
 */
 ...
 *
 * @deprecated The ESP32-S2 doesn't use I2S DMA. Call
 * ``adc_digi_output_format_t`` instead.
 */
typedef enum {
  ADC_ENCODE_12BIT, /*!< ADC to DMA data format,          , [15:12]-channel
                       [11:0]-12 bits ADC data */
  ADC_ENCODE_11BIT, /*!< ADC to DMA data format, [15]-unit, [14:11]-channel
                       [10:0]-11 bits ADC data */
  ADC_ENCODE_MAX,
} adc_i2s_encode_t;
 ...
 *
 * The default ADC voltage is for attenuation 0 dB and listed in the table
 * below. By setting higher attenuation it is possible to read higher voltages.
 *
 * Due to ADC characteristics, most accurate results are obtained within the
 * "suggested range" shown in the following table.
 *
 *     +----------+-------------+-----------------+
 ...
 *     +----------+-------------+-----------------+
 *
 * For maximum accuracy, use the ADC calibration APIs and measure voltages
 * within these recommended ranges.
 *
 * @note For any given channel, this function must be called before the first
 * time ``adc1_get_raw()`` is called for that channel.
 *
 * @note This function can be called multiple times to configure multiple
 *       ADC channels simultaneously. You may call ``adc1_get_raw()`` only after
 ...
 * @note ESP32:
 *       When the power switch of SARADC1, SARADC2, HALL sensor and AMP sensor
 * is turned on, the input of GPIO36 and GPIO39 will be pulled down for about
 * 80ns. When enabling power for any of these peripherals, ignore input from
 * GPIO36 and GPIO39. Please refer to section 3.11 of
 * 'ECO_and_Workarounds_for_Bugs_in_ESP32' for the description of this issue. As
 * a workaround, call adc_power_acquire() in the app. This will result in higher
 * power consumption (by ~1mA), but will remove the glitches on GPIO36 and
 * GPIO39.
 *
 ...
 * Configuring a new channel does not prevent a previously configured channel
 * from being read.
 *
 ...
 *
 * @note For any given channel, this function must be called before the first
 * time ``adc2_get_raw()`` is called for that channel.
 *
 ...
 *       must be called before the first time this function is called. If Wi-Fi
 * is started via ``esp_wifi_start()``, this function will always fail with
 * ``ESP_ERR_TIMEOUT``.
 *
 * @note ESP32-S2:
 *       ADC2 support hardware arbiter. The arbiter is to improve the use
 * efficiency of ADC2. After the control right is robbed by the high priority,
 *       the low priority controller will read the invalid ADC2 data. Default
 * priority: Wi-Fi > RTC > Digital;
 *
 ...
 * @return
 *     - ESP_OK if success
 *     - ESP_ERR_TIMEOUT ADC2 is being used by other controller and the request
 * timed out.
 *     - ESP_ERR_INVALID_STATE The controller status is invalid. Please try
 * again.
 */
...
---------------------------------------------------------------*/
/**
 * @brief Digital ADC DMA read max timeout value, it may make the
 * ``adc_digi_read_bytes`` block forever if the OS supports
 */
...
typedef struct adc_digi_init_config_s {
  uint32_t max_store_buf_size;  ///< Max length of the converted data that
                                ///< driver can store before they are processed.
  uint32_t conv_num_each_intr;  ///< Bytes of data that can be converted in 1
                                ///< interrupt.
...
typedef struct {
  bool conv_limit_en;  ///< To limit ADC conversion times. Conversion stops
                       ///< after finishing `conv_limit_num` times conversion
  uint32_t conv_limit_num;  ///< Set the upper limit of the number of ADC
                            ///< conversion triggers. Range: 1 ~ 255.
  uint32_t pattern_num;     ///< Number of ADC channels that will be used
  adc_digi_pattern_config_t
      *adc_pattern;  ///< List of configs for each ADC channel that will be used
  uint32_t
      sample_freq_hz; /*!< The expected ADC sampling frequency in Hz. Range:
                         611Hz ~ 83333Hz Fs = Fd / interval / 2 Fs: sampling
                         frequency; Fd: digital controller frequency, no larger
                         than 5M for better performance interval: interval
                         between 2 measurement trigger signal, the smallest
                         interval should not be smaller than the ADC measurement
                         period, the largest interval should not be larger than
                         4095 */
  adc_digi_convert_mode_t
      conv_mode;  ///< ADC DMA conversion mode, see `adc_digi_convert_mode_t`.
  adc_digi_output_format_t format;  ///< ADC DMA conversion output format, see
 ...
 *
 * @param init_config Pointer to Digital ADC initilization config. Refer to
 * ``adc_digi_init_config_t``.
 *
 * @return
 *         - ESP_ERR_INVALID_ARG   If the combination of arguments is invalid.
 *         - ESP_ERR_NOT_FOUND     No free interrupt found with the specified
 * flags
 ...
 * @param[out] out_length          Real length of data read from the ADC via
 * this API.
 * @param[in]  timeout_ms          Time to wait for data via this API, in
 * millisecond.
 *
 * @return
 *         - ESP_ERR_INVALID_STATE Driver state is invalid. Usually it means the
 * ADC sampling rate is faster than the task processing rate.
 *         - ESP_ERR_TIMEOUT       Operation timed out
 *         - ESP_OK                On success
 */
esp_err_t adc_digi_read_bytes(uint8_t *buf, uint32_t length_max,
                              uint32_t *out_length, uint32_t timeout_ms);

/**
 * @brief Start the Digital ADC and DMA peripherals. After this, the hardware
 * starts working.
 *
 * @return
...
/**
 * @brief Stop the Digital ADC and DMA peripherals. After this, the hardware
 * stops working.
 *
 * @return
 *         - ESP_ERR_INVALID_STATE Driver state is invalid.
 ...
 *
 * @param config Pointer to digital controller paramter. Refer to
 * ``adc_digi_config_t``.
 *
 * @return
 *      - ESP_ERR_INVALID_STATE Driver state is invalid.
 ...
 *
 * @note For ESP32S2, Filter IDX0/IDX1 can only be used to filter all enabled
 ...
 *      - ESP_OK Success
 */
esp_err_t adc_digi_filter_set_config(adc_digi_filter_idx_t idx,
                                     adc_digi_filter_t *config);
 ...
 *      - ESP_OK Success
 */
esp_err_t adc_digi_filter_get_config(adc_digi_filter_idx_t idx,
                                     adc_digi_filter_t *config);
 ...
 *
 * @note For ESP32S2, The monitor will monitor all the enabled channel data of
 * the each ADC unit at the same time.
 *
 ...
 *      - ESP_OK Success
 */
esp_err_t adc_digi_monitor_set_config(adc_digi_monitor_idx_t idx,
                                      adc_digi_monitor_t *config);
 ...
 *
 * @note When the power switch of SARADC1, SARADC2, HALL sensor and AMP sensor
 * is turned on, the input of GPIO36 and GPIO39 will be pulled down for about
 * 80ns. When enabling power for any of these peripherals, ignore input from
 * GPIO36 and GPIO39. Please refer to section 3.11 of
 ...
 * @param src I2S DMA data source, I2S DMA can get data from digital signals or
 * from ADC.
 * @return
 *     - ESP_OK success
 */
...
#define DMA_BUFFER_SIZE 		128 	//256
...
static int16_t adc2_chan_mask = 0;
static adc_channel_t channel[1] = {(adc_channel_t) ADC1_CHANNEL_0};
const char* ADC_DMA_TAG = "ADC_DMA";
esp_err_t ret;
uint32_t ret_num = 0;
...
{0}; dig_cfg.pattern_num = channel_num; for(int i = 0; i < channel_num; i++)
                        {
...
SOC_ADC_DIGI_MAX_BITWIDTH; ESP_LOGI(ADC_DMA_TAG, "adc_pattern[%d].atten is :%x",
i, adc_pattern[i].atten); ESP_LOGI(ADC_DMA_TAG, "adc_pattern[%d].channel is
:%x", i, adc_pattern[i].channel); ESP_LOGI(ADC_DMA_TAG, "adc_pattern[%d].unit is
:%x", i, adc_pattern[i].unit);
                        }
...
sizeof(channel) / sizeof(adc_channel_t)); adc_digi_start(); ret =
adc_digi_read_bytes(result, TIMES, &ret_num, ADC_MAX_DELAY); if (ret == ESP_OK
|| ret == ESP_ERR_INVALID_STATE)
                        {
                            ...
                            ESP_LOGI("TASK:", "ret is %x, ret_num is %d", ret,
ret_num); for (int i = 0; i < ret_num; i += ADC_RESULT_BYTE)
                            {
                                adc_digi_output_data_t *p =
(adc_digi_output_data_t*)&result[i]; #if CONFIG_IDF_TARGET_ESP32
                                ESP_LOGI(ADC_DMA_TAG, "Unit: %d, Channel: %d,
Value: %x", 1, p->type1.channel, p->type1.data); #endif
                            }
                            vTaskDelay(1);
                        }
                        else if (ret == ESP_ERR_TIMEOUT)
                        {
                            ESP_LOGW(ADC_DMA_TAG, "No data, increase timeout or
reduce conv_num_each_intr"); vTaskDelay(1000);
                        }
